...
// Phase 2 / 2-R6 (Login MFA) — generate a fresh base32 TOTP secret for enrollment.
export function generateTotpSecret(byteLength = 20): string {
  return encodeBase32(randomBytes(byteLength));
}

// Verify a user-entered TOTP code against a secret, allowing ±`window` periods of clock skew.
export function verifyTotpCode(
  secret: string,
  code: string,
  options: { window?: number; for_time?: number; digits?: number; period?: number } = {}
): boolean {
  ...
  const now = options.for_time ?? Date.now() / 1000;
  for (let step = -window; step <= window; step += 1) {
    let candidate: string;
    try {
      candidate = generateTotpCode(secret, { for_time: now + step * period, digits: options.digits, period });
    } catch {
      return false;
    }
    if (timingSafeEqualString(candidate, cleaned)) {
      return true;
    }
  }
  return false;
...
export function buildOtpAuthUri(secret: string, options: { issuer: string; account: string }): string {
  const label = encodeURIComponent(`${options.issuer}:${options.account}`);
  const params = new URLSearchParams({ secret: normalizeTotpSecret(secret), issuer: options.issuer });
...
function encodeBase32(buffer: Buffer): string {
  ...
  for (let offset = 0; offset + 5 <= bits.length; offset += 5) {
    out += alphabet[Number.parseInt(bits.slice(offset, offset + 5), 2)];
  }
  const remaining = bits.length % 5;
  if (remaining > 0) {
    out += alphabet[Number.parseInt(bits.slice(bits.length - remaining).padEnd(5, "0"), 2)];
  }
  return out;
}
