...
- If a reachable flow requires TOTP-based 2FA, use either `getOtpCodeFromFile(login, "storage/totp-secrets.txt")` or the local server API `POST /api/otp-accounts/code` only when the 2FA form is visible, then continue exploration and prefer saving auth state for the remaining crawl.
